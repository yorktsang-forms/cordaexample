...
class IOUContractStateApi : BaseStateRestApi<IOUState>(IOUState::class) {
    ...
    fun create(@RequestBody obj: CreatePayload) : String {
        return cordaService1.createIOU(obj.userPublicKey, Instant.now(), obj.loadRecords ?: emptyMap()).tx.id.toString()
    }

    @PutMapping("update/{ref}")
    fun update(@PathVariable("ref") ref : StateRef, @RequestBody obj: UpdatePayload) : String {
        val iouState = cordaService1.getState(ref, IOUState::class)
        ...
        return cordaService1.modifyIOU(ref, obj.loadRecords.associateBy( {it.sha256()}, {it.startDay})).tx.id.toString()
    }
}